...
export function DigitalTwin(props: {
  ...
  return (
    <figure aria-label="Stadium digital twin command map">
      <svg viewBox="0 0 640 520" role="img" aria-roledescription="crowd density heatmap">
        <title>Stadium crowd heatmap with generated action overlays</title>
        <desc>{overlaySummary()}</desc>
        <rect x="26" y="26" width="588" height="468" rx="28" fill="#eef7ef" stroke="#d5e4d6" />
        <For each={zones}>
          {(zone) => {
            const value = () => animatedDensity()[zone.id];
            const band = () => densityBand(value());
            return (
              <g aria-label={`${zone.label}: ${value() ?? "fallback"} percent, ${band()}`}>
                <rect
                  x={zone.x}
                  y={zone.y}
                  width={zone.w}
                  ...
                  stroke-width="2"
                />
                <text x={zone.x + zone.w / 2} y={zone.y + zone.h / 2} text-anchor="middle" font-weight="800">
                  {zone.label}
                </text>
              </g>
            );
        ...
        <For each={props.actions.filter((action) => action.mapOverlay)}>
          {(action) => (
            <g aria-label={`Action overlay: ${action.mapOverlay?.label}`}>
              <circle cx="544" cy="260" r="9" fill="#2578a6" stroke="#ffffff" stroke-width="3" />
              <path d="M544 300 C510 390 430 438 340 448" fill="none" stroke="#2578a6" stroke-width="5" />
