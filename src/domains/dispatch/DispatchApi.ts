...
export class DispatchApi {
  ...
  async dispatch(
    ...
    const approvalRequired = decision.actions.some(
      (action) => action.requiresApproval,
    );
    if (!approvalRequired)
      return { accepted: true, reason: "no_approval_required" };
    if (!signature)
      return { accepted: false, reason: "missing_ed25519_signature" };
    const verified = await this.auditChain.verify(
      decision,
      signature,
      operatorId,
    );
