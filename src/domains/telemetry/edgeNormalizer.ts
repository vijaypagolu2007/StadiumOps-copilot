...
export function normalizeTelemetry(frame: TelemetryFrame): { frame: TelemetryFrame; alerts: EdgeAlert[] } {
  ...
  for (const [zoneId, value] of Object.entries(frame.zones)) {
    ...
    if (band === "fallback") {
      alerts.push({
        ...
        summary: "Malformed or missing sensor value; route graph fallback is active.",
        zoneId: zoneId as EdgeAlert["zoneId"]
      });
    }
  }
